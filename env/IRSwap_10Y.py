...
class IRSwap_10Y(gym.Env):
    ...
    def _next_observation(self):
        # Get the stock data points for the last 3 days and scale to between 0-1
        frame = np.array([
    ...
    def _take_action(self, action):
        for i in range(10):
            ...
            self.delta[i] += self.delta_variation[i]
            noise = (1/200) * (2 * random.random() - 1)
            self.delta[i] += 1 + noise * B_total
